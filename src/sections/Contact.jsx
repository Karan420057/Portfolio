...
const Contact = () => {
  ...
  return (
    <section id='contact' className='w-full min-h-screen relative bg-black text-white overflow-hidden py-20 px-6 md:px-20 flex flex-col
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
            <div className='flex flex-col'>
              ...
              <input
                ...
                className={`p-3 rounded-md bg-white/10 border ${errors.name ? "border-red-500" : "border-gray-500"} text-white focus:outline-none fous:border-blue-500`}
              />
            ...
            <div class='flex flex-col'>
              ...
              <input
                ...
                onChange={handleChange}
                className={`p-3 rounded-md bg-white/10 border ${errors.email ? "border-red-500" : "border-gray-500"} text-white focus:outline-none fous:border-blue-500`}
              />
              {errors.email && <p className='text-red-500 text-xs'>{errors.email}</p>}
            </div>

            <div className='flex flex-col'>
              <label className='mb-1'>Seervice Needed <span className='text-red-500'>*</span></label>
              <select 
                name='service'
                value={formData.service}
                onChange={handleChange}
                className={`p-3 rounded-md bg-white/10 border ${errors.service ? "border-red-500" : "border-gray-500"} text-white focus:outline-none fous:border-blue-500`}
              >
            ...
            {formData.service && formData.service !== "other" && (
              <div className='flex flex-col'>
                ...
                <input
                  ...
                  className={`p-3 rounded-md bg-white/10 border ${errors.budget ? "border-red-500" : "border-gray-500"} text-white focus:outline-none fous:border-blue-500`}
                />
            ...
            <div className='flex flex-col'>
              ...
              <textarea name='idea'
                ...
                value={formData.idea}
                onChange={handleChange}
                className={`p-3 rounded-md bg-white/10 border ${errors.idea ? "border-red-500" : "border-gray-500"} text-white focus:outline-none fous:border-blue-500`}
              ></textarea>
            ...
            <motion.button
              ...
              whileTap={{scale: 0.95}}
              disaabed={status === "sending"}
              className='bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white py-3 rouded-md font-semibold transition'
            >{status === "sending" ? "Sending" : "Send Message"}</motion.button>
          </form>
        </motion.div>
      </div>
    </section>
